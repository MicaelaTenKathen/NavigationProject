...
def updateParticle_w(part, best, gp_best, g, GEN, phi1, phi2, phi3, wmin, wmax):
    u1 = np.array([random.uniform(0, phi1) for _ in range(len(part))])
    u2 = np.array([random.uniform(0, phi2) for _ in range(len(part))])
    u3 = np.array([random.uniform(0, phi3) for _ in range(len(part))])
    v_u1 = u1 * (part.best - part)
    v_u2 = u2 * (best - part)
...
toolbox.register("update_w", updateParticle_w, wmin=0.4, wmax=0.9)
...
x = abs(f_min) + abs(f_max)
y = abs(f_min) + abs(f_max)

while i < x:
    j = 0
    while j < y:
        x_grid.append(j - abs(f_min))
...
for part in pop:
    toolbox.update_w(part, best, gp_best, g, GEN, 2, 2, 0)
...
for g in range(GEN):
    ...
    for part in pop:
        toolbox.update_w(part, best, gp_best, g, GEN, phi1, phi2, phi3)
